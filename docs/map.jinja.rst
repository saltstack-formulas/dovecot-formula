...
We will see a quick setup to configure the ``dovecot`` formula for different DNS domain names and several roles.
...
Create per DNS configuration for ``dovecot`` formula
`````````````````````````````````````````````````````

Now, we can provides the per DNS domain name configuration files for the ``dovecot`` formulas under ``/srv/salt/dovecot/parameters/``.
...
.. code-block:: console
    ...
    mkdir -p /srv/salt/dovecot/parameters/dns:domain/
    ln -s dns:domain /srv/salt/dovecot/parameters/domain

We create a configuration for the DNS domain ``example.net`` in ``/srv/salt/dovecot/parameters/dns:domain/example.net.yaml``:
...
We create another configuration for the DNS domain ``example.com`` in the Jinja YAML template ``/srv/salt/dovecot/parameters/dns:domain/example.com.yaml.jinja``:
...
Create per role configuration for ``dovecot`` formula
``````````````````````````````````````````````````````

Now, we can provides the per role configuration files for the ``dovecot`` formulas under ``/srv/salt/dovecot/parameters/``.

We create the directory for roles:

.. code-block:: console

    mkdir -p /srv/salt/dovecot/parameters/roles
...
We create a configuration for the role ``dovecot/server`` in ``/srv/salt/dovecot/parameters/roles/dovecot/server.yaml``:
...
We create another configuration for the role ``dovecot/client`` in ``/srv/salt/dovecot/parameters/roles/dovecot/client.yaml``:
...
.. code-block:: console
    ...
    salt 'server-*' grains.append roles dovecot/server

For the clients:

.. code-block:: console

    salt 'client-*' grains.append roles dovecot/client
...
.. code-block:: console
    ...
    mv /srv/salt/dovecot/parameters/dns:domain/  '/srv/salt/dovecot/parameters/dns!domain/'
...
.. code-block:: sls
    ...
    {%- from tplroot ~ "/map.jinja" import mapdata as dovecot with context %}

    test-does-nothing-but-display-dovecot-as-json:
      test.nop:
        - name: {{ dovecot | json }}
...
Here is an example based on `template-formula/dovecot/config/file.sls`_:
    ...
    dovecot-config-file-file-managed:
      file.managed:
        ...
        - source: {{ files_switch(['example.tmpl'],
                                  lookup='dovecot-config-file-file-managed'
                     )
                  }}
        - mode: 644
        - user: root
        - group: {{ dovecot.rootgroup }}
        - makedirs: True
        - template: jinja
        - require:
          - sls: {{ sls_package_install }}
        - context:
            dovecot: {{ dovecot | json }}

This ``sls`` file expose a ``dovecot`` context variable to the jinja template which could be used like this:
...
.. code-block:: jinja
    ...
    winner of the merge: {{ dovecot['winner'] }}
...
.. _template-formula/dovecot/config/file.sls: https://github.com/saltstack-formulas/template-formula/blob/master/dovecot/config/file.sls
